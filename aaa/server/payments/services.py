...
class Signer:
    ...
    def _pre_sign(self, data: dict) -> str:
        ...
        if 'completeTime' in data:
            keys = ['mchCode', 'orderId', 'amount']
        else:
            keys = ['mchCode', 'orderId', 'amount', 'paymode', 'notifyUrl']
        join = []
        for key in keys:
            if 'amount' in key:
                s = f'"{key}":{data[key]}'
...
class Payment:
    ...
    def pay(self, payload):
        ...
        pay_type = payload.get('pay_type') or 'ALIPAY_QR'
        data = {
            'mchCode': self.mch_id,
            'orderId': order_no,
            'amount': amount,
            'paymode': pay_type,
            'notifyUrl': 'http://89e6-18-163-217-66.ngrok.io/test/notify',
            'user': '1',
            'payerName': '大老爸'
        }
